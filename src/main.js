...
const state = { cards:[], favorites:loadFavorites(), layout:loadLayout(), search:'', group:'전체', detailCard:null, detailRange:'90' };
const els = {
...
function asShortDate(value){if(!value)return''; const text=String(value); if(/^\d{4}-\d{2}$/.test(text))return text; const date=new Date(text); return Number.isNaN(date.getTime())?text.slice(0,10):date.toLocaleDateString('ko-KR',{year:'2-digit',month:'2-digit',day:'2-digit'});}
...
function formatTrend(card){const n=Number(card.trend90);if(!Number.isFinite(n))return '';if(card.trendMode==='bp')return `<span class="period-change ${n>0?'up':n<0?'down':''}">90D ${n>0?'+':''}${n.toFixed(0)}bp</span>`;return `<span class="period-change ${n>0?'up':n<0?'down':''}">90D ${n>0?'+':''}${n.toFixed(1)}%</span>`;}
...
function filteredCards(){let cards=state.cards.filter(card=>!state.layout.hidden.includes(card.id));const q=state.search.trim().toLowerCase();if(q)cards=cards.filter(card=>[card.title,card.group,card.status,card.note].join(' ').toLowerCase().includes(q));if(state.group!=='전체')cards=cards.filter(card=>card.group===state.group);if(state.layout.showOnlyFavorites)cards=cards.filter(card=>state.favorites.has(card.id));if(state.layout.sort==='priority')return [...cards].sort((a,b)=>b.priority-a.priority);if(state.layout.sort==='change')return [...cards].sort((a,b)=>Math.abs(b.change??-Infinity)-Math.abs(a.change??-Infinity));if(state.layout.sort==='name')return [...cards].sort((a,b)=>a.title.localeCompare(b.title,'ko'));if(state.layout.sort==='group')return [...cards].sort((a,b)=>a.group.localeCompare(b.group,'ko')||b.priority-a.priority);return orderedCards(cards);}

function render(){
  syncControls();const cards=filteredCards();els.empty.hidden=cards.length>0;
  els.grid.innerHTML=cards.map(card=>{
    const favorite=state.favorites.has(card.id);const clickable=card.historyAvailable?'has-history':'';
    return `<article class="metric-card tone-${escapeHtml(card.tone)} ${clickable}" data-id="${escapeHtml(card.id)}" draggable="${state.layout.sort==='custom'}" tabindex="${card.historyAvailable?'0':'-1'}" ${card.historyAvailable?'role="button" aria-label="'+escapeHtml(card.title)+' 상세 차트 보기"':''}>
      ...
      <div class="card-title-line"><h2>${escapeHtml(card.title)}</h2><span class="status-pill">${escapeHtml(card.status)}</span></div>
      ...
      <div class="card-foot"><span>${formatTrend(card)||escapeHtml(card.note||'')}</span><span>${escapeHtml(asShortDate(card.asOf))}</span></div>
    </article>`;
  }).join('');
}
...
async function openDetails(id,range='90'){
  const card=state.cards.find(item=>item.id===id);if(!card||!card.historyAvailable)return;
  state.detailCard=card;state.detailRange=range;els.detailGroup.textContent=card.group;els.detailTitle.textContent=card.title;els.detailValue.textContent=`${card.value}${card.unit}`;els.detailChange.innerHTML=formatChange(card.change);els.detailAsOf.textContent=card.asOf?`기준 ${asShortDate(card.asOf)}`:'';els.detailNote.textContent=card.note||'저장소에 누적된 정적 JSON에서 필요한 기간만 불러옵니다.';
  [...els.rangeTabs.querySelectorAll('button')].forEach(btn=>{if(btn.dataset.range==='24h')btn.hidden=!card.intradayAvailable;btn.classList.toggle('active',btn.dataset.range===range);});
  ...
  const {series}=await loadCardHistory(id,range);if(state.detailCard?.id!==id||state.detailRange!==range)return;els.detailChart.innerHTML=lineChart(series);
...
els.grid.addEventListener('keydown',event=>{if((event.key==='Enter'||event.key===' ')&&event.target.matches('.metric-card')){event.preventDefault();openDetails(event.target.dataset.id);}});
